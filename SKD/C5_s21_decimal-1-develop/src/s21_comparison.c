...
// Нормализация d1 и d2 путем уравнения их scale и копирования результата
void s21_normalize_scale_upper(s21_decimal *d, int norm) {
  ...
  // Прямое копирование результата в исходный decimal
  for (int i = 127; i >= 0; i--) {
    s21_set_bit(d, i, s21_get_bit(_res, i));
  }
}
